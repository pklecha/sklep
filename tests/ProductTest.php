...
class ProductTest extends AbstractDateBaseTest
{
    ...
    public function testLoadProductById()
    {
        $id = 1;
        $db = new Database();
        $productsRepository = new ProductRepository($db);
        $product = $productsRepository->findProductById($id);
        $this->assertEquals(1, $product->getId());
        $this->assertEquals('iPhone', $product->getName());
        $this->assertEquals(1, $product->getPhotoById(1)->getId());
        $this->assertEquals('/path/to/photo/1', $product->getPhotoById(1)->getPath());
    }
    ...
    public function testUpdateProductDetails()
    {
        ...
        $productsRepository = new ProductRepository($db);
        $product = $productsRepository->findProductById($id);
        $product->setName('Nokia3310');
        $product->setStock(5);
        $productsRepository->save($product);
        $product2 = $productsRepository->findProductById(1);
        $this->assertEquals(1, $product2->getId());
        $this->assertEquals('Nokia3310', $product2->getName());
    }
    ...
    public function testAddNewProduct()
    {
        ...
        $productsRepository = new ProductRepository($db);
        $product = new Product();
        $product->setName('Nokia6230');
        $product->setDescription('Used to be nice smartphone');
